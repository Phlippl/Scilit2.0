...
/**
 * PDF Processing service with improved error handling and chunking capabilities
 */
class PDFProcessingService {
   ...
   * @param {Function} progressCallback - Progress callback function (0-100)
   * @returns {Promise<Object>} - Promise resolving to extracted text and page info
   */
  async extractText(pdfFile, maxPages = 0, progressCallback = null) {
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        // Report progress
        if (progressCallback) {
          progressCallback(Math.round((i / pageCount) * 100));
        }
      }
  ...
  async processFile(file, options = {}) {
    ...
    try {
      ...
      reportProgress('text_extraction', 0);
      
      // Extract text from the PDF
      const extractionResult = await this.extractText(
        file, 
        maxPages,
        (percent) => reportProgress('text_extraction', percent)
      );
      
      // Extract identifiers
      reportProgress('metadata_extraction', 0);
      let doi = this.extractDOI(extractionResult.text);
      let isbn = this.extractISBN(extractionResult.text);
      
      // If no text was found or identifiers couldn't be extracted, try OCR
      let ocrText = '';
      if ((!doi && !isbn) || extractionResult.text.trim().length < 100) {
        reportProgress('ocr_processing', 0);
        
        // Only process first few pages to save time
        const ocrResult = await this.performOCR(
          file,
          [1, 2, 3], // First 3 pages
          (percent) => reportProgress('ocr_processing', percent)
        );
        ...
        reportProgress('ocr_processing', 100);
      }
